...
def run_task_transformation(args):
    logging.info("Runing task transformation (%s)." % args.transform_task)
    time_limit = limits.get_time_limit(
        args.translate_time_limit, args.overall_time_limit)
    memory_limit = limits.get_memory_limit(
        args.translate_memory_limit, args.overall_memory_limit)
    ...
    try:
        ...
        if err.errno == errno.ENOENT:
            returncodes.exit_with_driver_input_error("Error: {} not found.".format(executable))
        else:
