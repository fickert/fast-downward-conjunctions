...
public:
	...
	template<typename T>
	std::vector<T> get_enum_list(std::string key) const {
		auto enum_values = std::vector<T>();
		auto int_values = get_list<int>(key);
		enum_values.reserve(int_values.size());
		std::transform(std::begin(int_values), std::end(int_values), std::back_inserter(enum_values), [](int value) { return static_cast<T>(value); });
		return enum_values;
    }
