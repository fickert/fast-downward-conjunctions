...
#include "../grey/graph_algorithms/scc.h"
#include "../grey/graph_algorithms/topological_sort.h"
...
void RedBlackDAGFactFollowingHeuristic::prepare_DTGs_for_invertibility_check() {
    ...
    for (int op_no = 0; op_no < num_ops; ++op_no) {
		...
		for (size_t i = 0; i < effects.size(); i++) {
			...
			int pre_value = RedBlackOperator::get_precondition_for_variable(*op, var);
...
void RedBlackDAGFactFollowingHeuristic::set_black_variables() {
    ...
    if (num_blacks > 1 && paint_roots_black) {
        ...
        for (int i = 0; i < num_vars; ++i) {
            ...
            if (cg.get_predecessors(i).empty()) {
...
void RedBlackDAGFactFollowingHeuristic::set_black_variables_vertex_cover() {
    ...
    for (int i = 0; i < num_vars; ++i) {
		...
		auto neigh = get_cg_neighbours(cg, i);
...
void RedBlackDAGFactFollowingHeuristic::paint_red_by_vertex_cover(
    ...
    while (true) {
        ...
        auto neigh = get_cg_neighbours(cg, best_node);
        for (size_t n = 0; n < neigh.size(); ++n) {
            int neighbour = neigh[n];
            if (!black_vars[neighbour])
                continue;
...
auto RedBlackDAGFactFollowingHeuristic::get_cg_neighbours(const CausalGraph &cg, int var) -> std::vector<int> {
...
void RedBlackDAGFactFollowingHeuristic::get_relaxed_plan(const GlobalState &state,
        relaxation_heuristic::Proposition *goal) {
...
int RedBlackDAGFactFollowingHeuristic::get_semi_relaxed_plan_cost(
    ...
    red_black_plan.clear();
	for (const auto *goal_proposition : goal_propositions)
		add_subgoal(goal_proposition->fact.var, goal_proposition->fact.value);
#ifdef DEBUG_RED_BLACK
    ...
    while (true) {
        ...
        if (app_status == ACTION_APPLICABLE) {
            ...
            red_black_plan.push_back(op);
			for (const auto &precondition : op->get_preconditions())
				add_subgoal(precondition.var, precondition.val);
            // Set as preferred
...
int RedBlackDAGFactFollowingHeuristic::add_red_black_plan_suffix(
        const GlobalState &state, int h_val) {
...
const vector<int>& RedBlackDAGFactFollowingHeuristic::get_path_for_var(
    ...
    cout << "Current semi-relaxed state is: " << endl;
    dump_current_semi_relaxed_state(true);
//    GlobalState(curr_state_buffer).dump();
...
void RedBlackDAGFactFollowingHeuristic::set_op_as_preferred(const GlobalState &state,
...
void RedBlackDAGFactFollowingHeuristic::add_subgoal(int var, int val) {
	const auto fact = FactPair(var, val);
	if (std::find_if(std::begin(subgoals_and_costs), std::end(subgoals_and_costs), [&fact](const auto &subgoal_and_cost) {
		return subgoal_and_cost.first == fact;
	}) != std::end(subgoals_and_costs))
		return;
	assert(propositions[var][val].cost >= 0);
	if (propositions[var][val].cost == 0)
		return;
	subgoals_and_costs.emplace_back(fact, propositions[var][val].cost);
}
...
void RedBlackDAGFactFollowingHeuristic::add_options_to_parser(
	...
	parser.add_enum_option("prefs", pref_types, "preferred operators", "relaxed");
    ...
    parser.add_option<bool>("extract_plan_no_blacks",
            "attempts extracting plan from the heuristic solution", "false");
}
    ...
    if (next_red_action_test) {
        ...
        for (size_t op_no = 0; op_no < g_operators.size(); ++op_no) {
			...
			for (size_t i = 0; i < effects.size(); i++) {
				...
				if (black_vars[var])
					continue;
				int pre_value = RedBlackOperator::get_precondition_for_variable(g_operators[op_no], var);
				if (pre_value != -1) {
