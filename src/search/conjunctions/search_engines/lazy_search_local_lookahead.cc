#include "lazy_search_local_lookahead.h"
...
LazySearchLocalLookahead::LazySearchLocalLookahead(const options::Options &opts)
	: OnlineLearningSearchEngine(opts),
	  ...
	  exhaustive_lookahead(opts.get<bool>("exhaustive_lookahead")),
	  no_learning(opts.get<bool>("no_learning")),
	  enable_heuristic_cache(opts.get<bool>("enable_heuristic_cache")),
	  heuristic_cache() {
	auto lookahead_open_list_options = options::Options();
	lookahead_open_list_options.set("evals", get_evaluators(opts.get<int>("lookahead_weight"), false));
	lookahead_open_list_options.set("pref_only", false);
	lookahead_open_list_options.set("unsafe_pruning", true);
	lookahead_open_list = TieBreakingOpenListFactory(lookahead_open_list_options).create_edge_open_list();
}

void LazySearchLocalLookahead::set_pref_operator_heuristics(std::vector<Heuristic *> &heur) {
	preferred_operator_heuristics = heur;
}

void LazySearchLocalLookahead::set_iterated_weights_options(const options::Options &opts) {
...
auto LazySearchLocalLookahead::has_another_phase() const -> bool {
...
void LazySearchLocalLookahead::restart_with_next_weight() {
...
void LazySearchLocalLookahead::initialize() {
...
void LazySearchLocalLookahead::get_successor_operators(std::vector<const GlobalOperator *> &ops) {
	assert(ops.empty());
...
void LazySearchLocalLookahead::lookahead_expand(EvaluationContext &eval_context, SearchNode &node) {
	node.close();
	++lookahead_statistics.num_lookahead_expansions;
	const auto state = node.get_state();
	assert(!search_space.get_node(state).is_dead_end());
	assert(!eval_context.is_heuristic_infinite(conjunctions_heuristic));

	// we only consider preferred operators in the lookahead
	auto preferred_operators = eval_context.get_preferred_operators(conjunctions_heuristic);
	for (const auto *op : preferred_operators) {
		op->unmark();
		const auto new_g = node.get_g() + get_adjusted_cost(*op);
		const auto new_real_g = current_real_g + node.get_real_g() + op->get_cost(); // note the added current_real_g for comparison to the bound
		if (new_real_g <= bound) {
			auto new_eval_context = EvaluationContext(current_eval_context.get_cache(), new_g, true, nullptr);
			lookahead_open_list->insert(new_eval_context, std::make_pair(state.get_id(), op));
		}
	}
}

auto LazySearchLocalLookahead::lookahead() -> std::pair<SearchStatus, StateID> {
	++lookahead_statistics.num_lookahead;
	assert(lookahead_open_list->empty());
	...
	auto lookahead_root_eval_context = EvaluationContext(current_state, &statistics, true);
	const auto lookahead_root_h = evaluate_if_neccessary(lookahead_root_eval_context);
	assert(!lookahead_root_eval_context.is_heuristic_infinite(conjunctions_heuristic));
	lookahead_expand(lookahead_root_eval_context, initial_node);
	...
	while (!lookahead_open_list->empty()) {
		const auto entry = lookahead_open_list->remove_min();
		...
		const auto parent_node = lookahead_search_space->get_node(parent_state);
		...
		const auto h = evaluate_if_neccessary(eval_context);
		if (eval_context.is_heuristic_infinite(conjunctions_heuristic)) {
		...
		if ((h == 0 && test_goal(state)) || (!exhaustive_lookahead && best_state_h < lookahead_root_h))
			break;

		lookahead_expand(eval_context, node);
	}

	return {IN_PROGRESS, best_state_id};
}

void LazySearchLocalLookahead::generate_successors() {
...
SearchStatus LazySearchLocalLookahead::fetch_next_state() {
...
auto LazySearchLocalLookahead::generate_conjunction(SearchNode &node, SearchNode *global_lookahead_node) -> ConjunctionGenerationStatus {
...
SearchStatus LazySearchLocalLookahead::step() {
	// Invariants:
	...
	if (node.is_new() || node.is_open() || reopen) {
		...
		if (!open_list->is_dead_end(current_eval_context)) {
			...
			*/
			...
			lookahead_open_list->clear();

			// refine the heuristic once, then continue
			if (lookahead_state_id == StateID::no_state) {
				if (!no_learning) {
					switch (generate_conjunction(node)) {
				...
				if (do_learning_if_lookahead_closed == LookaheadClosedLearning::YES && global_lookahead_node.is_closed()) {
					...
					const auto lookahead_h = evaluate_if_neccessary(lookahead_eval_context);
					if (lookahead_eval_context.is_heuristic_infinite(conjunctions_heuristic) || lookahead_h >= h) {
						if (global_lookahead_node.is_closed()) {
							...
							lookahead_statistics.notify_lookahead_is_new_or_open(current_eval_context.get_heuristic_value(conjunctions_heuristic)
							                                                     - lookahead_eval_context.get_heuristic_value(conjunctions_heuristic));
						}
						if (!no_learning) {
							switch (generate_conjunction(node, &global_lookahead_node)) {
							case ConjunctionGenerationStatus::SOLVED:
						...
						// insert path to the best lookahead state into the open list
						auto lookahead_plan = Plan();
						lookahead_search_space->trace_path(lookahead_state, lookahead_plan);
...
auto LazySearchLocalLookahead::get_evaluators(int w, bool include_pref_eval) -> std::vector<ScalarEvaluator *> {
	...
	if (w == 0) {
		...
		evals.push_back(conjunctions_heuristic);
	} else {
		auto weighted_evaluator = new weighted_evaluator::WeightedEvaluator(conjunctions_heuristic, w);
...
void LazySearchLocalLookahead::update_eval_context(EvaluationContext &eval_context, const decltype(heuristic_cache)::mapped_type &cache_entry) {
...
auto LazySearchLocalLookahead::evaluate_if_neccessary(EvaluationContext &eval_context) -> int {
...
void LazySearchLocalLookahead::reward_progress() {
	open_list->boost_preferred();
}

void LazySearchLocalLookahead::print_checkpoint_line(int g) const {
	std::cout << "[g=" << g << ", ";
	statistics.print_basic_statistics();
	std::cout << "]" << std::endl;
}

void LazySearchLocalLookahead::print_statistics() const {
...
static void _add_lookahead_options(options::OptionParser &parser) {
	...
	parser.add_option<bool>("exhaustive_lookahead",
	                        "fully exhaust the lookahead search space instead of stopping at the first state that is better than the root state", "false");
...
static SearchEngine *_parse(options::OptionParser &parser) {
	...
	LazySearchLocalLookahead *engine = nullptr;
	if (!parser.dry_run()) {
		engine = new LazySearchLocalLookahead(opts);
...
static options::Plugin<SearchEngine> _plugin("lazy_ll", _parse);
static options::Plugin<SearchEngine> _plugin_greedy("lazy_greedy_ll", _parse_greedy);
static options::Plugin<SearchEngine> _plugin_weighted_astar("lazy_wastar_ll", _parse_weighted_astar);
static options::Plugin<SearchEngine> _plugin_iterated_weights("lazy_iterated_weights_ll", _parse_iterated_weights);
}
