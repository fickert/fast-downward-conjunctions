#include "enforced_hill_climbing_novelty_relaxed_subgoals_search_lazy.h"
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::create_open_list() -> open_list_t {
	return open_list_t([](const auto &lhs, const auto &rhs) {
		return lhs.g > rhs.g;
	});
}

EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy(const options::Options &opts) :
...
EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::~EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy() = default;

void EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::reach_state(const GlobalState &parent, const GlobalOperator &op, const GlobalState &state) {
	heuristic->notify_state_transition(parent, op, state);
	novelty_heuristic->notify_state_transition(parent, op, state);
}

void EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::initialize() {
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::get_applicable_ops(const GlobalState &state) -> std::vector<const GlobalOperator *> {
...
void EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::expand(SearchNode &node) {
	node.close();
	...
	for (const auto *op : applicable_ops) {
		...
		open_list.emplace(open_list_entry{state.get_id(), op, succ_g});
	}
}

auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::step() -> SearchStatus {
...
SearchStatus EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::ehc(SearchSpace &current_search_space) {
	...
	while (!open_list.empty()) {
		...
		auto [parent_state_id, op, g] = open_list.top();
		open_list.pop();
		...
		const auto state = state_registry.get_successor_state(parent_state, *op);
	...
	if (best_state_id != StateID::no_state) {
		auto state = state_registry.lookup_state(best_state_id);
		auto eval_context = EvaluationContext(state, &statistics);
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::evaluate_if_neccessary(EvaluationContext &eval_context, const GlobalState &parent_state, const GlobalOperator &last_op) -> int {
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::evaluate_if_neccessary(EvaluationContext &eval_context) -> int {
	const auto &result = eval_context.get_result(heuristic);
	return result.get_h_value();
}

// attempt to escape local minimum by generating conjunctions
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::escape_local_minimum(int target_h) -> SearchStatus {
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::handle_safe_dead_end() -> SearchStatus {
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::handle_search_space_exhaustion() -> SearchStatus {
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::escape_dead_end(const SearchNode &node) -> SearchStatus {
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::escape_potential_dead_end() -> SearchStatus {
	auto node = search_space.get_node(current_eval_context.get_state());
	assert(!node.is_dead_end());
	learning_stagnation_counter = 0;
	const auto parent_id = node.get_parent_state_id();
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::restart() -> SearchStatus {
	learning_stagnation_counter = 0;
...
auto EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::restart_in_parent() -> SearchStatus {
...
void EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::print_ehcc_statistics() const {
...
void EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::print_intermediate_statistics(const ConjunctionsHeuristic &heuristic) const {
	OnlineLearningSearchEngine::print_intermediate_statistics(heuristic);
	print_ehcc_statistics();
}


void EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy::print_statistics() const {
...
static auto _parse(OptionParser &parser) -> SearchEngine * {
	...
	if (parser.help_mode() || parser.dry_run())
		return nullptr;
	return new EnforcedHillClimbingNoveltyRelaxedSubgoalsSearchLazy(opts);
}

static Plugin<SearchEngine> _plugin("ehc_cnsgl", _parse);
}
