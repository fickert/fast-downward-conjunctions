#include "lazy_search_relaxed_subgoals_lookahead_bounded_expansions.h"
...
auto LazySearchRSLBoundedExpansions::create_open_list(int w) -> open_list_t {
	if (w == 0)
...
auto LazySearchRSLBoundedExpansions::get_lookahead_bound(const AbstractTask &task, const std::string &bound) -> int {
	if (bound != "facts") // FD converts arguments to lower case
		return std::stoi(bound);
	auto num_facts = 0;
...
LazySearchRSLBoundedExpansions::LazySearchRSLBoundedExpansions(const options::Options &opts)
	: OnlineLearningSearchEngine(opts),
	  ...
	  solved(false),
	  lookahead_bound(get_lookahead_bound(*g_root_task(), opts.get<std::string>("lookahead_bound"))),
...
void LazySearchRSLBoundedExpansions::set_pref_operator_heuristics(std::vector<Heuristic *> &heur) {
	preferred_operator_heuristics = heur;
}

void LazySearchRSLBoundedExpansions::set_iterated_weights_options(const options::Options &opts) {
...
auto LazySearchRSLBoundedExpansions::has_another_phase() const -> bool {
...
void LazySearchRSLBoundedExpansions::restart_with_next_weight() {
...
void LazySearchRSLBoundedExpansions::initialize() {
...
void LazySearchRSLBoundedExpansions::get_successor_operators(std::vector<const GlobalOperator *> &ops) {
	assert(ops.empty());
...
void LazySearchRSLBoundedExpansions::lookahead_expand(SearchNode &node) {
	node.close();
...
auto LazySearchRSLBoundedExpansions::lookahead() -> std::pair<SearchStatus, StateID> {
	...
	for (auto &bsg_node : conjunctions_heuristic->get_last_bsg().nodes)
	...
	auto current_lookahead_expansions = 1;

	auto best_state_id = StateID::no_state;
	auto best_state_h = std::numeric_limits<int>::max();

	while (!lookahead_open_list.empty() && current_lookahead_expansions++ <= lookahead_bound) {
...
void LazySearchRSLBoundedExpansions::generate_successors() {
...
SearchStatus LazySearchRSLBoundedExpansions::fetch_next_state() {
...
SearchStatus LazySearchRSLBoundedExpansions::step() {
	// Invariants:
...
void LazySearchRSLBoundedExpansions::update_eval_context(EvaluationContext &eval_context, const decltype(heuristic_cache)::mapped_type &cache_entry) {
...
auto LazySearchRSLBoundedExpansions::evaluate_if_neccessary(EvaluationContext &eval_context) -> int {
...
void LazySearchRSLBoundedExpansions::reward_progress() {
	open_list->boost_preferred();
}

void LazySearchRSLBoundedExpansions::print_checkpoint_line(int g) const {
	std::cout << "[g=" << g << ", ";
	statistics.print_basic_statistics();
	std::cout << "]" << std::endl;
}

void LazySearchRSLBoundedExpansions::print_statistics() const {
...
static void _add_lookahead_options(options::OptionParser &parser) {
	...
	parser.add_option<std::string>("lookahead_bound", "maximum number of expansion in each lookahead (integer or FACTS for the number of facts)", "FACTS");
...
static SearchEngine *_parse(options::OptionParser &parser) {
	...
	LazySearchRSLBoundedExpansions *engine = nullptr;
	if (!parser.dry_run()) {
		engine = new LazySearchRSLBoundedExpansions(opts);
...
static options::Plugin<SearchEngine> _plugin("lazy_rsl_be", _parse);
static options::Plugin<SearchEngine> _plugin_greedy("lazy_greedy_rsl_be", _parse_greedy);
static options::Plugin<SearchEngine> _plugin_weighted_astar("lazy_wastar_rsl_be", _parse_weighted_astar);
static options::Plugin<SearchEngine> _plugin_iterated_weights("lazy_iterated_weights_rsl_be", _parse_iterated_weights);
}
