#include "lazy_search_yahsp_lookahead.h"
...
void LazySearchYAHSPLookahead::set_iterated_weights_options(const options::Options &opts) {
...
auto LazySearchYAHSPLookahead::has_another_phase() const -> bool {
...
void LazySearchYAHSPLookahead::restart_with_next_weight() {
	...
	if (node.is_new() || node.is_open() || reopen) {
		...
		if (!open_list->is_dead_end(current_eval_context)) {
			...
			*/
			...
			auto lookahead_eval_context = EvaluationContext(lookahead_state, current_g + lookahead_plan.size(), true, &statistics);
...
void LazySearchYAHSPLookahead::update_eval_context(EvaluationContext &eval_context, const decltype(heuristic_cache)::mapped_type &cache_entry) {
...
auto LazySearchYAHSPLookahead::evaluate_if_neccessary(EvaluationContext &eval_context) -> int {
...
static options::Plugin<SearchEngine> _plugin("lazy_yahsp", _parse);
static options::Plugin<SearchEngine> _plugin_greedy("lazy_greedy_yahsp", _parse_greedy);
static options::Plugin<SearchEngine> _plugin_weighted_astar("lazy_wastar_yahsp", _parse_weighted_astar);
static options::Plugin<SearchEngine> _plugin_iterated_weights("lazy_iterated_weights_yahsp", _parse_iterated_weights);
}
