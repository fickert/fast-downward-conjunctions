...
void EnforcedHillClimbingSearch::initialize() {
	...
	solved = generate_conjunctions(*heuristic, ConjunctionGenerationStrategy::Event::INITIALIZATION, current_eval_context) == ConjunctionGenerationStrategy::Result::SOLVED;
	...
	if (!solved && check_relaxed_plans && is_valid_plan_in_the_original_task(heuristic->get_last_bsg(), current_eval_context.get_state().get_values(), *g_root_task())) {
		solved = true;
...
SearchStatus EnforcedHillClimbingSearch::ehc(SearchSpace &current_search_space) {
	...
	auto result = generate_conjunctions(*heuristic, ConjunctionGenerationStrategy::Event::START_EHC_PHASE, current_eval_context);
	switch (result) {
	...
	while (!open_list->empty()) {
		...
		if (node.is_new()) {
			...
			if (check_relaxed_plans && heuristic->is_last_bsg_valid_for_state(state) && is_valid_plan_in_the_original_task(heuristic->get_last_bsg(), state.get_values(), *g_root_task())) {
	...
	if (!k_cutoff && preferred_usage != PreferredUsage::PRUNE_BY_PREFERRED) {
...
auto EnforcedHillClimbingSearch::escape_local_minimum() -> SearchStatus {
	...
	if (is_learning_stagnation) {
		...
		case LearningStagnation::BACKJUMP:
			// restart search in the parent state
		...
		case LearningStagnation::RESTART:
			// restart search in the initial state
			if (current_eval_context.get_state().get_id() != state_registry.get_initial_state().get_id())
				return restart();
			break;
		case LearningStagnation::CONTINUE:
			// continue with learning as normal
			break;
		default:
			std::cerr << "Unknown repeated learning option." << std::endl;
			utils::exit_with(utils::ExitCode::CRITICAL_ERROR);
		}
	}

	is_learning_stagnation = true;

	do {
		current_eval_context = EvaluationContext(current_eval_context.get_state(), &statistics);
		auto learning_result = generate_conjunctions(*heuristic, ConjunctionGenerationStrategy::Event::LOCAL_MINIMUM, current_eval_context);
		...
		if (!k_cutoff)
			return handle_search_space_exhaustion();
	} while (!(always_reevaluate || conjunctions_strategy->deletes_conjunctions()) && current_eval_context.get_heuristic_value(heuristic) <= bfs_lowest_h_value);
...
auto EnforcedHillClimbingSearch::handle_search_space_exhaustion() -> SearchStatus {
	...
	switch (search_space_exhaustion) {
	case SearchSpaceExhaustion::CONTINUE:
		return no_learning ? FAILED : IN_PROGRESS;
	case SearchSpaceExhaustion::RESTART:
		return restart();
	case SearchSpaceExhaustion::BACKJUMP:
		return escape_potential_dead_end();
	default:
		std::cerr << "Unknown search space exhaustion option." << std::endl;
		utils::exit_with(utils::ExitCode::CRITICAL_ERROR);
	}
}
...
void EnforcedHillClimbingSearch::print_ehcc_statistics() const {
	std::cout << "EHC Phases: " << num_ehc_phases << std::endl;
...
void EnforcedHillClimbingSearch::print_statistics() const {
	...
	std::cout << "EHC phases: " << num_ehc_phases << std::endl;
	if (num_ehc_phases != 0)
		std::cout << "Average expansions per EHC phase: " << static_cast<double>(statistics.get_expanded()) / num_ehc_phases << std::endl;
...
static auto _parse(OptionParser &parser) -> SearchEngine * {
	...
	parser.add_enum_option("learning_stagnation", {"CONTINUE", "RESTART", "BACKJUMP"},
		"search behavior whenever after learning there is no better successor in the BFS neighborhood within the bound k", "CONTINUE");
	...
	std::vector<std::string> preferred_usages;
